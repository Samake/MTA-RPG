...
float4 MTACalcGTABuildingDiffuseWithBrightness(float4 InDiffuse, float brightness) {
    float4 OutDiffuse;
...
float4 MTACalcGTAVehicleDynamicDiffuse( float3 WorldNormal, float3 InLightDirection, float4 InDiffuse, float4 InAmbient)
{
    // Calculate diffuse color by doing what D3D usually does
    float4 ambient  = gAmbientMaterialSource  == 0 ? gMaterialAmbient  : InDiffuse;
    float4 diffuse  = gDiffuseMaterialSource  == 0 ? gMaterialDiffuse  : InDiffuse;
    ...
    float4 TotalAmbient = ambient * ( InAmbient + gLightAmbient );

    // Add the strongest light
    float DirectionFactor = max(0.2, dot(WorldNormal, InLightDirection ));
    float4 TotalDiffuse = (diffuse * DirectionFactor );
